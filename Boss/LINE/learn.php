   ...
   $talkreply = "不能輸入空值。\n格式 :\n老大學#關鍵字#回答句";
      ...
      $talkreply = "你輸入的內容包含禁止使用文字。";
         ...
         $talkreply = "你輸入的內容包含禁止使用文字。";
            ...
            if (strpos($learnword, ",") !== false) {$learnword = explode(",", $code[1]);}else{if (strpos($learnword, "，") !== false) {$learnword = explode("，", $code[1]);}}
            if (strpos($replyfromlearn, ",") !== false) {$replyfromlearn = explode(",", $code[2]);}}else{if (strpos($replyfromlearn, "，") !== false) {$replyfromlearn = explode("，", $code[2]);}}
